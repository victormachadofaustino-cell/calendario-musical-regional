// src/components/Dashboard/IndicadoresTab.jsx // Localização: A gaveta oficial dos dados técnicos da orquestra.
...
import { 
  ...
  AlertTriangle, ChevronRight, Info, Trophy, Users, X
...
const IndicadoresTab = ({ todosEnsaios, ensaiosRegionais, examinadoras, encarregados, user }) => { // Início do componente técnico.
  ...
  useEffect(() => { // Lógica que busca dados analíticos extras no banco de dados.
    ...
    return () => { unsubAlt(); }; // Desliga a consulta ao sair da tela para economizar dados.
  ...
  return ( // Início da estrutura visual da página de indicadores.
    <div className="space-y-8 animate-in">
      ...
      <div className="space-y-3">
        <div className="flex justify-between items-center px-2"><h4 className="text-[9px] font-black uppercase text-slate-400 tracking-widest">{indexGraficoRegional === 0 ? 'Frequência Mensal (Agenda)' : 'Ocorrências por Sede'}</h4><div className="flex gap-1"><button onClick={() => setIndexGraficoRegional(0)} className={`w-1.5 h-1.5 rounded-full ${indexGraficoRegional === 0 ? 'bg-amber-500' : 'bg-slate-200'}`} /><button onClick={() => setIndexGraficoRegional(1)} className={`w-1.5 h-1.5 rounded-full ${indexGraficoRegional === 1 ? 'bg-amber-500' : 'bg-slate-200'}`} /></div></div>
        <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm min-h-[340px] overflow-hidden relative">
          <AnimatePresence mode="wait">
            <motion.div key={indexGraficoRegional} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} drag="x" dragConstraints={{ left: 0, right: 0 }} onDragEnd={(e, info) => { if (info.offset.x < -50 && indexGraficoRegional < 1) setIndexGraficoRegional(1); if (info.offset.x > 50 && indexGraficoRegional > 0) setIndexGraficoRegional(0); }} className="h-[260px] w-full cursor-grab active:cursor-grabbing">
